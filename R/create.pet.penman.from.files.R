...
#'
#' @export
create.pet.penman.from.files <- function(input.files, output.file, author.data,  climdex.time.resolution="daily", axis.to.split.on="Y", parallel=4,
  ...
  variable.name.map = variable.name.map[c('tmin', 'tmax', 'ss', 'rs', 'ws', 'rh', 'vp', 'dp')]
...
#browser()
    ...
    return(lapply(r, function(x) {
      ...
      concrete_args$ss = concrete_args$ss[,x]
      ...
      return(PETr::pet_penmon(concrete_args))
    }))
  }
