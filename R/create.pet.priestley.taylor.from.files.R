## Run PETr to estimate PET using Priestley-Taylor method
#' Create PET
#'
#' Create PET
#'
...
#'
#' @export
create.pet.priestly.taylor.from.files <- function(input.files, output.file, author.data,  climdex.time.resolution="daily", axis.to.split.on="Y", parallel=4,
  ...
  variable.name.map = variable.name.map[c('tmin', 'tmax', 'rs', 'rh', 'vp', 'dp')]
  ...
  compute.pet.for.stripe <- function(subset, ts, dim.axes, v.f.idx, variable.name.map) {
    ...
    return(lapply(r, function(x) {
      ...
      concrete_args$vp = concrete_args$vp[,x]
      concrete_args$dp = concrete_args$dp[,x]
      concrete_args$lons <- concrete_args$lons[[x]]
      return(PETr::pet_priestley_taylor(concrete_args))
    }))
  }
