...
#'
#' @param scPDSI.file The filename to be used for the scPDSI file.
...
#' @param scPDSI.dat scPDSI metadata
...
#'
#' @export
create.scPDSI.file <- function(scPDSI.file, f, ts, v.f.idx, variable.name.map, dim.size, dim.axes, scPDSI.dat, author.data) {
  ...
  # Define the data
  out.var = ncdf4::ncvar_def( "scPDSI", units="index", list(dimX,dimY,dimT), mv, prec="float", longname = "Self Calibrating Palmer Drought Index")

  ############################################################################
  ## Create file
  scPDSI.netcdf <- ncdf4::nc_create(scPDSI.file, out.var, force_v4=TRUE)

  return(scPDSI.netcdf)
}
